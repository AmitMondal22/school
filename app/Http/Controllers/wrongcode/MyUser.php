...
class MyUser extends Controller
{
    ...
    public function my_profile(Request $request){
        $gurd_name=Auth::getDefaultDriver();
        $id=auth()->guard($gurd_name)->user()->wc_user_id;
        if($request->isMethod('post')){
            ...
            if($valaditor->fails()){
                // return response()->json($valaditor->errors(),401); //400 envalies responce
                Toastr::success($valaditor->errors(), 'Title', ["positionClass" => "toast-bottom-right"]);
                return redirect()->route('wc.sa_profile');
            }
            ...
            $data=Wc_user::where('wc_user_id', $id)->update([
                ...
                'dp_path'=>$path,
                'user_role'=>$request->userrole,
                'update_by'=>auth()->guard($gurd_name)->user()->wc_user_id
        ...
        return view('wrongcode_admin.auth.my_profile')->with($dataa);
    }
    }
    ...
    public function change_my_profile_password(Request $request){
        $gurd_name=Auth::getDefaultDriver();
        $id=auth()->guard($gurd_name)->user()->wc_user_id;
            ...
            $userpassword=Wc_user::select('password')->where('wc_user_id', $id)->first()->password;
            if (Hash::check($request->user_password, $userpassword)) {
            $data=Wc_user::where('wc_user_id', $id)->update([
